...
적용해 8개 도구로 재구성했습니다. 문서 원본 파일 다운로드(document.xml, corpCode.xml
원본, XBRL 원본파일 등 바이너리 응답)는 당초 JSON 데이터 조회 목적과 맞지 않아 라이브
도구로는 제외했습니다(단, corpCode.xml은 회사명 검색 기능 내부 구현에 활용).

**2026-08-08 예외 추가**: document.xml만 `dart_get_document_text`로 승격했습니다. 정형
8개 도구가 다루지 않는 서술형 정보(사업의 내용·계열회사·정관 목적·주석·별첨)를 "빈
결과"로 오인해 존재하는 정보를 "확인 불가"로 잘못 보고하는 사례가 반복되어, 이 경로를
스킬 문서 대신 도구 자체에 내장하기로 함(자세한 배경은 커밋 로그 참고). corpCode.xml·
XBRL 원본은 이미 다른 방식(정적 캐시/불필요)으로 해결돼 있어 예외에서 제외.

## 제공 도구 (9개 — 정형 8개 + 원문 조회 1개)
...
9. **dart_get_document_text** — 공시서류 원본파일(document.xml) 조회. 정형 8개 도구가
   다루지 않는 서술형 정보(사업의 내용·계열회사 현황·정관상 사업목적과 영위 여부·주요
   계약·주석·별첨) 전용. `rcept_no`(접수번호) 필요. mode=outline(TITLE 목차+SUMMARY
   요약수치)/count(키워드 등장횟수만, 부존재 증명용)/extract(키워드 주변 텍스트,
   occurrenceIndex로 다중 발견 위치 선택). 원문은 요청 처리 중에만 메모리에 존재하고
   저장하지 않으며, 응답도 추출된 일부만 반환한다. 2026-08-08 신설.
...
- `dart_get_document_text` 라이브 배포 검증(포니링크 사업보고서 rcept_no
  20260318000966): outline 모드로 파일 3개(본문 5,021,276자 + 첨부 2건) 식별, count
  모드로 "건설업" 0건·"전기공사업" 1건·"계열회사" 29건 확인(사업보고서 원문 수기
  카운트 기록과 일치), extract 모드로 "전기공사업" 항목이 정관 목적 제17호로 "미영위"
  표시된 문맥까지 확인.
- 초기 배포분은 15초×3회 재시도 방식이었으나 매번 52초 내외에서 타임아웃 실패 —
  corpCode.xml(3.5MB) 대용량 다운로드 지연과 동일 계열 증상으로 판단해 50초 단일
  시도로 변경 후 34~36초에 정상 응답하는 것으로 재검증함.
