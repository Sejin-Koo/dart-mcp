...
async function loadCorpCodeList() {
  ...
  const t1 = Date.now();
  console.log(`[dart-mcp] corpCode fetch() 응답 헤더 수신: ${t1 - t0}ms, status=${res.status}`);
  if (!res.ok) throw new Error(`DART corpCode.xml HTTP ${res.status}`);
  const buf = Buffer.from(await res.arrayBuffer());
  const t2 = Date.now();
  console.log(`[dart-mcp] arrayBuffer 다운로드 완료: ${t2 - t1}ms, size=${buf.length} bytes`);
  const zip = new AdmZip(buf);
  const entry = zip.getEntries()[0];
  const xml = entry.getData().toString("utf-8");
  const t3 = Date.now();
  console.log(`[dart-mcp] unzip 완료: ${t3 - t2}ms, xml length=${xml.length}`);
  ...
  //
  // 성능 주의: 약 10만+ 건을 처리해야 하므로 정규식을 블록/필드마다 새로 만들면(new RegExp)
  // 로컬 샌드박스에서는 버틸 만해도(약 1.4초) Vercel 프로덕션에서는 30초 제한을 넘기는
  // 실제 문제가 발생했다(2026-07-25 배포 후 확인). indexOf/slice만 쓰는 아래 방식으로
  // 정규식 컴파일 자체를 없애 고쳤다.
  const list = [];
  ...
  while (true) {
    ...
    list.push({
      corp_code: extractTag(block, "corp_code"),
      corp_name: extractTag(block, "corp_name"),
      stock_code: extractTag(block, "stock_code"),
      modify_date: extractTag(block, "modify_date"),
    });
    pos = end + LIST_CLOSE.length;
  }
  const t4 = Date.now();
  console.log(`[dart-mcp] indexOf 파싱 완료: ${t4 - t3}ms, entries=${list.length}, 전체 소요: ${t4 - t0}ms`);
