...
// scripts/refresh-corp-code.mjs에 기록된 실측(corpCode.xml 3.5MB가 Vercel 함수에서
// 60초를 넘겨도 안 끝나는 대용량 다운로드 스로틀링)과 동일 계열 증상이 document.xml
// (사업보고서 기준 zip 500KB대)에서도 실측 확인됨(2026-08-08) — 짧은 시도를 여러 번
// 반복해도 매번 타임아웃만 나고 실패했다. 재시도 횟수보다 한 번의 시도에 예산을 몰아주는
// 쪽이 낫다: maxDuration=60초 예산 안에서 단일 시도에 대부분을 쓰고, unzip·파싱·응답
// 오버헤드용으로 약 8초만 남긴다.
...
async function downloadZip(rcept_no) {
  ...
  throw new Error(
    `document.xml 다운로드 실패: ${lastErr?.message}. ` +
      `대용량 파일(zip)은 opendart.fss.or.kr↔Vercel 구간에서 간헐적으로 지연·스로틀링이 ` +
      `발생할 수 있습니다(scripts/refresh-corp-code.mjs에 corpCode.xml 3.5MB 사례 기록). ` +
      `이 도구 호출을 다시 시도해서 풀리지 않으면, Bash로 curl 직접 호출(man-dart 6-2)로 ` +
      `전환하세요 — 세션마다 네트워크 경로가 달라 이 경로가 오히려 더 빠를 수 있습니다: ` +
...
function unzipEntries(buf) {
  ...
  return zip.getEntries().map((e) => {
    const { text, encoding } = decodeXml(e.getData());
    return { name: e.entryName, xml: text, encoding };
  });
}

// 인코딩 자동 판별 — 2026-08-08 실측으로 확인된 사항:
// 최근 제출분(예: 2026-03 사업보고서)은 UTF-8이지만, 구버전 제출분(예: 2021-03 제출
// 사업보고서 rcept_no=20210319001198)은 EUC-KR(CP949)이다. UTF-8로 고정 디코딩하면
// 구버전 문서는 한글이 전부 U+FFFD로 깨지고, 그 결과 태그명조차 매칭되지 않아
// 키워드 카운트가 전부 0건으로 나온다("문서에 없음"으로 오인하기 쉬운 실패 형태).
// 따라서 UTF-8로 먼저 디코딩해 깨짐 비율을 보고, 과도하면 EUC-KR로 재디코딩한다.
const REPLACEMENT_CHAR = "\uFFFD";

function decodeXml(buf) {
  const utf8 = new TextDecoder("utf-8").decode(buf);
  // 표본만 검사(앞부분 200KB)해도 충분하며, 대용량 문서에서 전체 스캔 비용을 피한다.
  const sample = utf8.slice(0, 200_000);
  let bad = 0;
  for (const ch of sample) if (ch === REPLACEMENT_CHAR) bad++;
  const badRatio = sample.length ? bad / sample.length : 0;
  // UTF-8 문서에도 원문 자체에 깨진 글자가 드물게 섞일 수 있어 0이 아닌 임계값을 둔다.
  if (badRatio < 0.001) return { text: utf8, encoding: "utf-8" };

  try {
    const euckr = new TextDecoder("euc-kr").decode(buf);
    const euckrSample = euckr.slice(0, 200_000);
    let euckrBad = 0;
    for (const ch of euckrSample) if (ch === REPLACEMENT_CHAR) euckrBad++;
    const euckrRatio = euckrSample.length ? euckrBad / euckrSample.length : 0;
    // 더 적게 깨지는 쪽을 채택 (둘 다 나쁘면 최소한 덜 나쁜 쪽)
    if (euckrRatio < badRatio) return { text: euckr, encoding: "euc-kr" };
  } catch {
    // TextDecoder가 euc-kr을 지원하지 않는 런타임이면 UTF-8 결과를 그대로 사용
  }
